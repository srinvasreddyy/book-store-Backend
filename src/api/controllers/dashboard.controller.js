...
const getDashboardStats = asyncHandler(async (req, res) => {
  const [userCount, bookCount, orderCount, totalRevenueResult] =
    await Promise.all([
      User.countDocuments(),
      Book.countDocuments(),
...
const getAdminBookStats = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, stats, "Admin book statistics fetched successfully"),
    );
});
...
const getAdminSalesStats = asyncHandler(async (req, res) => {
  ...
  const salesData = await Order.aggregate([
    ...
    {
      $match: {
        "items.uploadedBy": new mongoose.Types.ObjectId(adminId),
        status: "COMPLETED",
      },
    },
    {
