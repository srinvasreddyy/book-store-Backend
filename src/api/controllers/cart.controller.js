...
const getCart = asyncHandler(async (req, res) => {
    ...
    if (!cart) {
        ...
        return res.status(200).json(new ApiResponse(200, newCart, "Cart is empty. A new cart has been created for you."));
    }
...
const addItemToCart = asyncHandler(async (req, res) => {
    ...
    if (book.stock < totalQuantity) {
        throw new ApiError(400, `Not enough stock. Only ${book.stock} items are available, but you are trying to add ${totalQuantity}.`);
    }
...
const clearCart = asyncHandler(async (req, res) => {
    ...
    if (!cart) {
        ...
        const newCart = await Cart.create({ user: req.user._id, items: [] });
        return res.status(200).json(new ApiResponse(200, newCart, "Cart is already empty."));
    }
